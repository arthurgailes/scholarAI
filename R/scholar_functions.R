...
generate_scholar_function <- function(
  ...
  # Input validation
  if (missing(scholar_name) || !is.character(scholar_name) || length(scholar_name) != 1) {
  ...
  if (missing(prompt_path) || !file.exists(prompt_path)) {
...
generate_scholar_functions <- function(
  ...
  if (is.null(authors) || is.null(db_path) || is.null(prompt_path) || is.null(output_dir)) {
    if (progress) cli::cli_alert_info("Loading configuration from {.file {config_path}}")
    ...
    if (file.exists(config_path)) {
      ...
      if (is.null(authors) && !is.null(config$authors)) authors <- config$authors
      if (is.null(db_path) && !is.null(config$db_path)) db_path <- config$db_path
      if (is.null(prompt_path) && !is.null(config$prompt_path)) prompt_path <- config$prompt_path
      if (is.null(output_dir) && !is.null(config$output_dir)) output_dir <- config$output_dir
    }
  }
  
  # Validate required parameters
  if (is.null(authors)) stop("Authors must be provided or available in config")
  if (is.null(db_path)) stop("Database path must be provided or available in config")
  if (is.null(output_dir)) stop("Output directory must be provided or available in config")
  ...
  if (is.null(prompt_path)) {
    ...
    if (file.exists(default_prompt_path)) {
      ...
      if (progress) cli::cli_alert_info("Using inferred prompt path: {.path {prompt_path}}")
    } else {
  ...
  custom_content <- c(
    ...
    "# Generated by the scholarAI package on ", format(Sys.time(), "%Y-%m-%d %H:%M:%S"),
    "",
  ...
  for (author in authors) {
    ...
    if (progress) {
      cli::cli_alert_info("Creating function {.fn {function_name}} for {author_name}")
    }
    ...
    custom_content <- c(
      custom_content,
      paste0(function_name, " <- function(query, limit = 5, temperature = 0.7, progress = TRUE) {"),
      "  scholarAI::ask_scholar(",
      paste0("    query = query,"),
      ...
      "}",
      "",
      paste0("# Example usage: ", function_name, "(\"What are your thoughts on housing policy?\")")
    )
    
    if (progress) {
      cli::cli_alert_success("Created function {.fn {function_name}} in global environment")
    }
  }
  
  # Write the custom.R file
  writeLines(custom_content, custom_file)
  if (progress) cli::cli_alert_success("Created custom.R file with scholar functions")
