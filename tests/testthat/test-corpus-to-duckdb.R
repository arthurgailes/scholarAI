test_that("corpus_to_duckdb creates database with expected structure", {
  ...
  mock_metadata <- data.frame(
    ...
    folder = c(
      file.path(temp_dir, "article1"),
      file.path(temp_dir, "article2")
    ),
  ...
  yyjsonr::write_json_file(mock_metadata, metadata_json, auto_unbox = TRUE)
  ...
  expect_true(all(grepl(
    ...
    corpus_table$content
  )))

  # Test that we can query by folder
  folder_query <- DBI::dbGetQuery(
    con,
...
test_that("corpus_to_duckdb handles missing files gracefully", {
  ...
  writeLines(
    ...
    file.path(mock_metadata$folder[1], "text.txt")
  )
  dir.create(mock_metadata$folder[2], recursive = TRUE)
  # don't create second to trigger warning
...
test_that("corpus_to_duckdb handles large text content", {
  ...
  large_text <- paste(
    rep("This is a test of large content.", 5000),
    collapse = ""
  )
  ...
  expect_equal(nchar(corpus_table$content[1]), nchar(large_text), tol = 0.01)
})
