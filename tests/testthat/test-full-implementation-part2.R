...
test_that("Scholar functions can be generated and loaded", {
  # Skip if OpenAI API key is not available
  skip_if_not(openai_avail, "OpenAI API key not available")
  skip_if_not(openrouter_avail, "OpenRouter API key not available")
  ...
  expect_true("query" %in% names(fn_args), "askTobias missing 'query' argument")
  expect_true("limit" %in% names(fn_args), "askTobias missing 'limit' argument")
  expect_true(
    "temperature" %in% names(fn_args),
    "askTobias missing 'temperature' argument"
  )
})

# Only run this test if explicitly requested with an environment variable
# This prevents the test from failing in CI environments or when API keys aren't working
if (Sys.getenv("SCHOLARAI_RUN_API_TESTS") == "true") {
  test_that("Scholar functions can be run with real API calls", {
    ...
    # Check if askTobias function is available
    if (!exists("askTobias")) {
      # Source the custom file if needed
      custom_file <- file.path(out_dir, "custom.R")
      source(custom_file)
    }

    # Run the askTobias function with a simple query
    test_query <- "What are your thoughts on housing policy?"
    message("Testing askTobias with query: '", test_query, "'")

    # Use tryCatch to handle potential errors gracefully
    response <- tryCatch(
      {
        askTobias(test_query, limit = 2, temperature = 0.7, progress = TRUE)
      },
      error = function(e) {
        message("Error running askTobias: ", e$message)
        return(NULL)
      }
    )
  ...
  test_that("Scholar functions API test is skipped", {
    skip("SCHOLARAI_RUN_API_TESTS environment variable not set to 'true'")
  })
}
