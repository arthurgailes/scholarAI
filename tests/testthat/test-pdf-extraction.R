...
test_that("handle_pdfs extracts PDFs from iframes", {
  ...
  # Download the page
  res <- httr::GET(report_url, httr::config(ssl_verifypeer = FALSE), httr::timeout(20))
  ...
  # Call handle_pdfs
  pdf_saved <- scholarAI:::handle_pdfs(main_article, report_url, test_dir, 
  ...
  if (length(pdf_files) > 0) {
    ...
    expect_true(file_info$size > 1000, "PDF file should have meaningful content (>1KB)")
  }
})
...
test_that("extract_and_save includes pdf_saved status", {
  ...
  expect_true("pdf_saved" %in% names(result), 
              "extract_and_save result should include pdf_saved status")
  expect_true(result$pdf_saved, 
  ...
  expect_true(dir.exists(folder_path), 
              "Article folder should be created")
  ...
  expect_true(length(sep_line) == 1, "PDF text separator should be present in article_text.txt")
...
test_that("copy_pdfs correctly copies PDFs to backup directory", {
  ...
  expect_equal(readLines(file.path(target_dir, "article1_1.pdf")), "PDF content 1")
  expect_equal(readLines(file.path(target_dir, "article2_1.pdf")), "PDF content 3")
})
